...
function calcButtonClick(newValue) {
    ...
    switch (newValue) {
        case '%':
            ...
            // На всякий случай форматируем число, чтобы исправить проблему с представлением числа
            // Например, 0.99/100 = 0.0098(9)
            if (fraction && fraction[0].length>1) {
                dividend = dividend.toFixed(fraction[0].replace('.', '').length + 2);
            }
        ...
        // Добавляем остальные числа [1-9]. Если уже решили пример, то начинается новый, иначе продолжаем набор.
        default:
